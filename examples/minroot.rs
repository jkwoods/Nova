...
fn main() {
  ...
  for num_iters_per_step in [1024, 2048, 4096, 8192, 16384, 32768, 65536] {
    ...
    let mut circuit = MinRootCircuit {
      seq: vec![
        MinRootIteration {
    ...
    let mut recursive_snark: RecursiveSNARK<E1, E2, C> =
      RecursiveSNARK::<E1, E2, C>::new(&pp, &mut minroot_circuits[0], &z0, None, vec![], vec![])
        .unwrap();

    for (i, mut circuit) in minroot_circuits.into_iter().enumerate() {
      let start = Instant::now();
